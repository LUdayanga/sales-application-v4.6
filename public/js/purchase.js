$(document).ready(function () {
    ...
    //set qc input value when enter purchase qty value
    $(document).on("keyup", "#qc_quantity", function () {
        $(this).closest('tr').find("td:eq(3) input").val($(this).val());
        // $("#add_qc_qty").val($(this).closest('tr').find("td:eq(2) input").val());
    });

    $(document).on("keyup", "#edit_purchase_qty", function () {
        $(this).closest('tr').find("td:eq(3) input").val($(this).val());
        // $("#edit_qc_qty").val($(this).val());
    });

    //set max value in qc qty
    $(document).on("keyup", "#edit_qc_qty", function () {
        if (parseFloat($(this).val()) > parseFloat($(this).closest('tr').find("td:eq(2) input[type='text']").val())) {
            $(this).val('');
        }
    });
    ...
    $('#supplier_id').select2({
        ...
        $('#advance_balance_text').text(currency_symbol + ' ' + data.balance, true);
    ...
    $(document).on('change', '.profit_percent', function () {
        ...
        var exchange_rate = $('input#currency_rate').val();
        __write_number(
...
function update_table_total() {
    ...
    $('#total_subtotal').text(currency_symbol + ' ' + total_subtotal);
...
function update_grand_total() {
    ...
    $('#discount_calculated_amount').text(currency_symbol + ' ' + discount, true, true);
    ...
    $('#tax_calculated_amount').text(currency_symbol + ' ' + tax, true, true);
    ...
    $('#grand_total').text(currency_symbol + ' ' + grand_total, true, true);

    // $('#payment_due').text(__currency_trans_from_en(due, true, true));
    $('#payment_due').text(currency_symbol + ' ' + due, true, true);
...
$(document).on('change', 'input.payment-amount', function () {
    ...
    $('#payment_due').text(currency_symbol + ' ' + bal, true, true);
});
...
$("#purchase_order_ids").on("select2:select", function (e) {
    ...
    $("#po_no").val('');
    $("#po_no").val($("#purchase_order_ids option:selected").text());
...
function set_po_values(po) {
    ...
    $('#select_curenncy').val(po.selected_currency_symbol).change();
    $('#selected_currency').val(po.selected_currency);
    $('#txt_currency_symbol').val(po.selected_currency_symbol);
    $('#currency_rate').val(po.exchange_rate);
