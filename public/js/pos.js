$(document).ready(function () {
    ...
    $(document).on('change', '#select_curenncy', function () {
        // toggle_search();

        // var curr_id = $(this).val();
        var curr_id = $('option:selected', this).attr('data');

        $.ajax({
            method: 'post',
            url: '/system-currentcies/get-details',
            data: { curr_id: curr_id },

            success: function (response) {
                $('#selected_currency').val(response.name);
                $('#txt_currency_symbol').val(response.symbol);
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function () {
        var currency_symbol = $('#txt_currency_symbol').val();
        ...
        tr.find('span.pos_line_total_text').text(currency_symbol + ' ' + line_total, true);
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price', function () {
        ...
        let last_purchased_price = __number_uf(tr.find('input#last_purchased_price').val());


        var unit_price_inc_tax = __add_percent(discounted_unit_price, tax_rate);
        if (last_purchased_price > unit_price_inc_tax) {
            tr.find('span#purchase_price_msg').attr('hidden', false);
        } else {
            tr.find('span#purchase_price_msg').attr('hidden', true);
        }
...
function pos_total_row() {
    ...
    $('table#pos_table tbody tr').each(function () {
        ...
        var tax_rate = $(this)
            ...
            .data('rate');
        var unit_price_inc_tax = __add_percent(discounted_unit_price, tax_rate);
        if (unit_price_inc_tax >= last_purchased_price) {
            price_ok_count++;
        }

    });

    let allow_selling_below_purchase_price = true;
    if ($('input#allow_selling_below_purchase_price').length) {

        allow_selling_below_purchase_price = false;

    }


    if (allow_selling_below_purchase_price) {


        if (actual_product_count === price_ok_count) {

            $('#submit-sell').attr('disabled', false);
            $('#save-and-print').attr('disabled', false);
            $('#pos-finalize').attr('disabled', false);
            $('.pos-express-finalize').attr('disabled', false);
        } else {
            $('#submit-sell').attr('disabled', true);
            $('#save-and-print').attr('disabled', true);
            $('#pos-finalize').attr('disabled', true);
            $('.pos-express-finalize').attr('disabled', true);
        }
    }
    ...
    $('span.price_total').html(currency_symbol + ' ' + price_total, false);
...
function calculate_balance_due() {
    ...
    $('span.total_paying').text(currency_symbol + ' ' + total_paying, true);

    __write_number($('input#in_balance_due'), bal_due);
    // $('span.balance_due').text(__currency_trans_from_en(bal_due, true));
    $('span.balance_due').text(currency_symbol + ' ' + bal_due, true);
...
$('table#pos_table tbody').on('change', 'input.pos_line_total', function () {
    ...
    tr.find('span.pos_line_total_text').text(currency_symbol + ' ' + subtotal, true);
...
function set_so_values(so) {
    $('#select_curenncy').val(so.selected_currency_symbol).change();
    $('#selected_currency').val(so.selected_currency);
    $('#txt_currency_symbol').val(so.selected_currency_symbol);
    $('#currency_rate').val(so.exchange_rate);
