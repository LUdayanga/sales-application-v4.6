$(document).ready(function () {
    ...
    $(document).on("change", ".lot_number", function () {
        if ($(this).prop('selectedIndex') != 0) {
            $(this).closest('tr').find("td:eq(4) input[type='text']").val(parseFloat($(this).find(':selected').attr('data-qty_available')).toFixed(2));
            $(this).closest('tr').find("td:eq(5) input[type='number']").val(parseFloat($(this).find(':selected').attr('data-qty_available')).toFixed(2));
            $(this).closest('tr').find("td:eq(6) input[type='number']").val(parseFloat($(this).find(':selected').attr('data-qty_available')).toFixed(2));
        }else{
            $(this).closest('tr').find("td:eq(4) input[type='text']").val('');
            $(this).closest('tr').find("td:eq(5) input[type='number']").val('');
            $(this).closest('tr').find("td:eq(6) input[type='number']").val('');
        }
    });

    //set max value in qc qty
    $(document).on("keyup", "#input_qc_qty", function () {
        if (parseFloat($(this).val()) > parseFloat($(this).closest('tr').find("td:eq(4) input[type='text']").val())) {
            $(this).val('');
        }
    });
