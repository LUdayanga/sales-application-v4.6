...
<tr class="product_row" data-row_index="{{ $row_count }}"
    ...
    <td>
        ...
        <span id="purchase_price_msg" hidden style="color: red;">Below Purchased Price</span>
        <input type="hidden" id="last_purchased_price" value="{{ $product->last_purchased_price }}">
        ...
        @if (session()->get('business.enable_lot_number') == 1 || session()->get('business.enable_product_expiry') == 1)
            ...
            @endphp
            @if (empty($is_sales_order))
                <select class="form-control lot_number input-sm text-center"
                    name="products[{{ $row_count }}][tr_sell_lot_number]" required>
                    <option value="">@lang('Select Badge Numbers')</option>
                    @foreach ($product->badge_numbers as $lot_number)
                        @php
                            // $selected = "";
                            ...
                            $max_qty_msg = __('lang_v1.quantity_error_msg_in_lot', ['qty' => $lot_number->qty_available, 'unit' => $product->unit]);
                            // }
                            ...
                            // }
                        @endphp
                        <option value="{{ $lot_number->lot_number }}"
                            data-qty_available="{{ $lot_number->qty_available }}" data-msg-max="@lang('lang_v1.quantity_error_msg_in_lot', ['qty' => $lot_number->qty_available, 'unit' => $product->unit])">
                            {{ $lot_number->lot_number }} </option>
                    @endforeach
                </select>
            @endif
        @endif
    ...
    <td>
        ...
        <div class="input-group input-number">
			...
			@else 
                ...
                @if ($product->enable_stock && empty($pos_settings['allow_overselling']) && empty($is_sales_order)) data-rule-max-value="" data-qty_available="" data-msg-max-value="" 
				data-msg_max_default="" @endif>
    ...
    <td class="text-center">
        ...
        <span class="display_currency pos_line_total_text @if (!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif"
            data-currency_symbol="false">{{ $product->quantity_ordered * $unit_price_inc_tax }}</span>
    </td>
