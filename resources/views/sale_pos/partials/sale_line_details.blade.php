<table class="table bg-gray">
        ...
        @if( session()->get('business.enable_lot_number') == 1)
            <th class="text-center">{{ __('Badge Number') }}</th>
        @endif
        @if($sell->type == 'sales_order')
            <th class="text-center">@lang('lang_v1.quantity_remaining')</th>
        @endif
        <th class="text-center">{{ __('sale.qty') }}</th>
        @if(!empty($pos_settings['inline_service_staff']))
            <th class="text-center">
                @lang('restaurant.service_staff')
            </th>
        @endif
        <th class="text-center">{{ __('sale.unit_price') }}</th>
        <th class="text-center">{{ __('sale.discount') }}</th>
        <th class="text-center">{{ __('sale.tax') }}</th>
        <th class="text-center">{{ __('sale.price_inc_tax') }}</th>
        <th class="text-center">{{ __('sale.subtotal') }}</th>
    </tr>
    @foreach($sell->sell_lines as $sell_line)
        <tr>
            ...
            @if( session()->get('business.enable_lot_number') == 1)
                <td>{{ $sell_line->tr_sell_lot_number ?? '--' }}
