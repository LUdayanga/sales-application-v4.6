@extends('layouts.app')
@section('title', __('Add Goods Received Note'))
...
<section class="content-header">
    <h1>@lang('Add Goods Received Note') <i class="fa fa-keyboard-o hover-q text-muted" aria-hidden="true" data-container="body" data-toggle="popover" data-placement="bottom" data-content="@include('purchase.partials.keyboard_shortcuts_details')" data-html="true" data-trigger="hover" data-original-title="" title=""></i></h1>
</section>
		...
		<div class="row">
			...
			<div class="col-sm-3 @if(!empty($default_purchase_status)) hide @endif">
				<div class="form-group">
					...
					{!! Form::select('status', $addPurchaseOrderStatus, $default_purchase_status, ['class' => 'form-control select2', 'required']); !!}
				</div>
			</div>	
			...
			<div class="col-sm-3" style="display: none">
				<div class="form-group">
					...
					{!! Form::select('location_id', $business_locations, $default_location, ['class' => 'form-control select2', 'required'], $bl_attributes); !!}
				</div>
			</div>
			...
			<div class="col-sm-3 @if(!$currency_details->purchase_in_diff_currency) hide @endif">
				<div class="form-group">
					...
					<div class="input-group">
						...
						{!! Form::number('', $currency_details->p_exchange_rate, ['class' => 'form-control', 'required', 'step' => 0.001]); !!}
					</div>
		...
		<div class="row">
			<div class="col-sm-3">
				<div class="form-group">
					...
					{!! Form::select('purchase_order_ids[]', [], null, ['class' => 'form-control select2', 'multiple', 'id' => 'purchase_order_ids', 'required']); !!}
				</div>
			</div>
			<div class="col-sm-3">
				<input type="text" id="po_no" name="po_no" hidden>
			</div>
		</div>
		@endif
		...
		<div class="row">
			<div class="col-sm-3">
				<div class="form-group">
					...
					<select class="form-control select2" name="selected" id="select_curenncy">
						@foreach ($system_currencies as $system_currency)
							<option data="{{ $system_currency->id }}" value="{{ $system_currency->currency_symbol }}">{{ $system_currency->currency_name . ' - ' . $system_currency->currency_symbol }}</option>
						@endforeach
					</select>
					<input type="hidden" name="selected_currency" id="selected_currency" value="{{ $default_currency_rate->currency_name }}">
					<input type="hidden" name="selected_currency_symbol" id="txt_currency_symbol" value="{{ $default_currency_rate->currency_symbol }}">
				</div>
			</div>
			<div class="col-sm-3">
				<div class="form-group">
					<label for="">Exchange Rate:</label>
					@if (auth()->user()->can('currency_rate.edit'))
					<input type="number" class="form-control" name="exchange_rate" id="currency_rate" value="{{ $default_currency_rate->currency_rate }}">
					@else
					<input type="number" class="form-control" name="exchange_rate" id="currency_rate" value="{{ $default_currency_rate->currency_rate }}" readonly>
					@endif
				</div>
			</div>
		</div>
					...
					<table class="table table-condensed table-bordered table-th-green text-center table-striped" id="purchase_entry_table">
						<thead>
							<tr>
								...
								<th style="display: none">@lang( 'QC passed quantity' )</th>
