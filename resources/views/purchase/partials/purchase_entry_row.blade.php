    ...
    <tr @if (!empty($purchase_order_line)) data-purchase_order_id="{{ $purchase_order_line->transaction_id }}" @endif>
        ...
        <td>
            ...
            @if ($product->enable_stock == 1)
                ...
                <br>
                <small class="text-muted" style="white-space: nowrap;">@lang('report.current_stock'): @if ($current_quantity > 0)
                        {{ $current_quantity }}
                    @else
                        0.00
                    @endif {{ $product->unit->short_name }}</small>
                <br>
                <small class="text-muted" style="white-space: nowrap;">@lang('Needs for total production'): @if ($need_for_sale > 0)
                        {{ $need_for_sale }}
                    @else
                        0.00
                    @endif {{ $product->unit->short_name }}</small>
                ...
                @if ($remaining_qty > 0)
                    <br>
                    <small class="text-muted" style="white-space: nowrap;">@lang('Remaining quantity'): @if ($remaining_qty > 0)
                            {{ $remaining_qty }}
                        @else
                            0.00
                        @endif {{ $product->unit->short_name }}</small>
        ...
        <td>
            ...
            @php
                ...
                $max_quantity = !empty($purchase_order_line) ? $purchase_order_line->quantity - $purchase_order_line->qc_qty : 0;
            @endphp

            <input type="text" name="purchases[{{ $row_count }}][quantity]"
                value="{{ @format_quantity($quantity_value) }}"
                class="form-control input-sm purchase_quantity input_number mousetrap" id="add_purchase_qty" required
        ...
        <td style="display: none;">
            <input type="text" name="purchases[{{ $row_count }}][qc_quantity]" value="0.00"
                class="form-control input-sm qc_quantity input_number mousetrap" id="" required readonly
                data-rule-abs_digit={{ $check_decimal }}
                data-msg-abs_digit="{{ __('lang_v1.decimal_value_not_allowed') }}"
                @if (!empty($max_quantity)) data-rule-max-value="{{ $max_quantity }}"
                    data-msg-max-value="{{ __('lang_v1.max_quantity_quantity_allowed', ['quantity' => $max_quantity]) }}" @endif>
        ...
        <td>
            ...
            @endphp
            {!! Form::text('purchases[' . $row_count . '][pp_without_discount]', number_format($pp_without_discount, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm purchase_unit_cost_without_discount input_number', 'required']) !!}
        </td>
        <td>
        ...
        <td>
            {!! Form::text('purchases[' . $row_count . '][purchase_price]', number_format($purchase_price, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'form-control input-sm purchase_unit_cost input_number', 'required']) !!}
        </td>
        <td class="{{ $hide_tax }}">
        ...
        @if (empty($is_purchase_order))
            ...
            @if (session('business.enable_lot_number'))
                <td>
                    {!! Form::text('purchases[' . $row_count . '][lot_number]', null, ['class' => 'form-control input-sm', 'required']) !!}
                </td>
            @endif
