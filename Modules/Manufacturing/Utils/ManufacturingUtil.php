...
class ManufacturingUtil extends Util
{
    ...
    public function getIngredientDetails($recipe, $business_id, $location_id = null)
    {
        ...
        foreach ($ingredient_variations as $ingredient_variation) {
            ...
            $check_location = BusinessLocation::skip(1)->take(1)->first();

            $total_qty = VariationLocationDetails::where('product_id',$product_id)
                                            ->where('variation_id', $variation_id)
                                            ->where('location_id', $check_location->id)
                                            ->sum('qty_available');
            ...
            $ingredients[] = [
                ...
                'ingredient_type' => $ingredient_variation->ingredient_type
           ];
        }
    ...
    public function getRecipeTotal($row)
    {
        ...
        foreach ($row->ingredients as $ingredient) {
            if (!empty($ingredient->variation) && $ingredient->ingredient_type == 'raw') {
