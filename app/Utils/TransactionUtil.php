...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'total_before_tax' => $invoice_total['total_before_tax'] * $input['exchange_rate'],
            ...
            'discount_amount' => $uf_data ? $this->num_uf($input['discount_amount'] * $input['exchange_rate']) : $input['discount_amount'] * $input['exchange_rate'],
            'tax_amount' => $invoice_total['tax'] * $input['exchange_rate'],
            'final_total' => $final_total * $input['exchange_rate'],
            ...
            'selected_currency' => isset($input['selected_currency']) ? $input['selected_currency'] : null,
            'selected_currency_symbol' => isset($input['selected_currency_symbol']) ? $input['selected_currency_symbol'] : null,
        ...
        if(!empty($input['change_return'])) {
                        $transaction->change_return = $input['change_return'];
                        $transaction->save();
                    }
                    if(!empty($input['total_paying_input'])) {
                        $transaction->total_paying_input = $input['total_paying_input'];
                        $transaction->save();
                    }
     ...
     */
    public function createOrUpdateSellLines($transaction, $products, $location_id, $exchange, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                            ...
                            if (!empty($product['modifier_sell_line_id'][$key])) {
                                ...
                                if (!empty($product['modifier_price'][$key])) {
                                    ...
                                    $modifiers_formatted[] = new TransactionSellLine([
                                        ...
                                        'unit_price_before_discount' => $this_price * $exchange,
                                        'unit_price' => $this_price * $exchange,
                                        'unit_price_inc_tax' => $this_price * $exchange,
                ...
                $line = [
                    ...
                    'tr_sell_lot_number' =>  !empty($product['tr_sell_lot_number']) ? $product['tr_sell_lot_number'] : null,
                    'unit_price_before_discount' => $unit_price_before_discount * $exchange,
                    'unit_price' => $unit_price * $exchange,
                    'line_discount_type' => !empty($product['line_discount_type']) ? $product['line_discount_type'] : null,
                    'line_discount_amount' => !empty($product['line_discount_amount']) ? $uf_data ? $this->num_uf($product['line_discount_amount'] * $exchange) : $product['line_discount_amount'] : 0,
                    'item_tax' =>  ($uf_item_tax / $multiplier) * $exchange,
                    'tax_id' => $product['tax_id'],
                    'unit_price_inc_tax' =>  ($uf_unit_price_inc_tax / $multiplier) * $exchange,
                    ...
                    'ingredient_type' => !empty($product['ingredient_type']) ? $product['ingredient_type'] : null,
                ];
    ...
    public function createOrUpdatePaymentLines($transaction, $payments, $business_id = null, $user_id = null, $uf_data = true)
    {
        ...
        $exchange = $transaction->exchange_rate;
        ...
        foreach ($payments as $payment) {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    $payment_data = [
                        'amount' => $payment_amount * $exchange,
                    ...
                    $cheque_inputs['cheque_date'] = $paid_on;
                    $cheque_inputs['cheque_issued_date'] = $uf_data ? $this->uf_date($payment['cheque_issued_date'], true) : $payment['cheque_issued_date'];
                    $cheque_inputs['cheque_amount'] = $payment_amount * $exchange;
                    ...
                    ChequeTransaction::create([
                        'cheque_amount' => $payment_amount * $exchange,
                        'cheque_id' => $cheque->id,
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($il->show_customer == 1) {
            ...
            $customer_mobile = [];
                ...
                if (!empty($customer->landline)) {
                    $customer_mobile[] = $customer->landline;
                }

            
            $output['customer_mobile_info'] = implode(' ', $customer_mobile);
            ...
            if (!empty($customer->address_line_2)) {
                $customer_address[] = $customer->address_line_2;
            }
            ...
            //delivery info
            if (!empty($customer->address_line_1)) {
                $output['customer_delivery_address_1'] = $customer->address_line_1;
            }
            if (!empty(trim($customer->address_line_2))) {
                $output['customer_delivery_address_2'] = $customer->address_line_2;
            }
            if (!empty($customer->city)) {
                $output['customer_delivery_address_3'] = $customer->city;
            }
        ...
        if($transaction->sub_status != null &&  $transaction->sub_status == 'quotation'){
            $output['transaction_status'] = 'quotation';
        }else if ($transaction->sub_status != null &&  $transaction->sub_status == 'proforma') {
            $output['transaction_status'] = 'proforma';
        }else{
            $output['transaction_status'] = $transaction->status;
        }
        ...
        if (blank($il->date_time_format)) {
            ...
            $output['invoice_date'] = date('d-M-y', strtotime($transaction->transaction_date));
        } else {
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            foreach ($details['lines'] as $line) {
                ...
                $total_price_discount += $p_discount;
        ...
        $output['total_discount'] = $discount + $total_price_discount;
        //reward points
        ...
        if (!empty($transaction->is_export)) {
          ...
          //change return
        $output['change_return'] = $transaction->change_return;
        $output['total_paying_input'] = $transaction->total_paying_input;
        ...
        foreach ($lines as $line) {
            ...
            if (!empty($line->sub_unit->short_name)) {
               ...
               // ourprice
                 ...
                 $our_price = '--';
                 $price_discount = 0;
                 if($variation->sell_price_inc_tax > $line->unit_price) {
                     $unit_price = $this->num_f($variation->sell_price_inc_tax, false, $business_details);
                     $our_price = $this->num_f($line->unit_price, false, $business_details);
                     $price_discount = ($variation->sell_price_inc_tax - $line->unit_price) * $line->quantity;
                 }
    ...
    */
     ...
     * @return void
     */
    public function payAtOnce($parent_payment, $type,$for_opening_balance = false)
    {
            ...
            foreach ($due_transactions as $transaction) {
                ...
                if ($total_amount > 0) {
                    ...
                    $array = [
                            ...
                            'cheque_id' => !empty($parent_payment->cheque_id) ? $parent_payment->cheque_id : null,
                            'created_at' => $now,
                            'updated_at' => $now
                        ];
                ...
                if ($parent_payment->method == 'cheque') {
                    ...
                    ChequeTransaction::create([
                        ...
                        'cheque_id' => $parent_payment->cheque_id,
                        'transaction_id' => $transaction->id,
    ...
    public function getLotNumbersFromVariation($variation_id, $business_id, $location_id, $exclude_empty_lot = false)
    {
        ...
        if ($exclude_empty_lot) {
            $query->whereRaw('(purchase_lines.quantity_sold + purchase_lines.quantity_adjusted + purchase_lines.quantity_returned) < purchase_lines.qc_qty');
        } else {
            $query->whereRaw('(purchase_lines.quantity_sold + purchase_lines.quantity_adjusted + purchase_lines.quantity_returned) <= purchase_lines.qc_qty');
        }

        $purchase_lines = $query->select('purchase_lines.id as purchase_line_id', 'lot_number', 'purchase_lines.exp_date as exp_date', DB::raw('(purchase_lines.qc_qty - (purchase_lines.quantity_sold + purchase_lines.quantity_adjusted + purchase_lines.quantity_returned)) AS qty_available'))->get();
    ...
    */
    ...
    public function createExpense($request, $business_id, $user_id, $format_data = true)
    {
        ...
        $transaction_data['final_total'] = $format_data ? $this->num_uf($transaction_data['final_total'] * $request->input('exchange_rate')) : $transaction_data['final_total'] *  $request->input('exchange_rate');
        ...
        $transaction_data['total_before_tax'] = $transaction_data['final_total'] * $request->input('exchange_rate');
        ...
        $transaction_data['selected_currency'] = $request->input('selected_currency');
        $transaction_data['selected_currency_symbol'] = $request->input('selected_currency_symbol');
        $transaction_data['exchange_rate'] = $request->input('exchange_rate');
    ...
    public function updateExpense($request, $id, $business_id, $format_data = true)
    {
        ...
        $exchange = $this->num_uf($request->input('exchange_rate'));

        $final_total = $request->has('final_total') ? $request->input('final_total') : $transaction->final_total;
        if ($request->has('final_total')) {
            $transaction_data['final_total'] = $format_data ? $this->num_uf($final_total * $exchange) : $final_total * $exchange;
    ...
    public function payContact($request, $format_data = true)
    {
        ...
        if ($inputs['method'] == 'cheque') {
            $cheque = Cheque::create([
                    ...
                    'cheque_date' =>  Carbon::parse($request->get('paid_on'))->format('Y-m-d h:m:s'),
                    'cheque_issued_date' => Carbon::parse($request->get('cheque_issued_date'))->format('Y-m-d h:m:s'),
                    'cheque_amount' => $this->num_uf($inputs['amount']),
                    'cheque_status' => 'due',
                    'cheque_type' => $request->get('cheque_type'),
        ...
        if ($request->get('for_opening_balance') == true) {
           $inputs['transaction_type'] = 'opening_balance';
        }
        event(new TransactionPaymentAdded($parent_payment, $inputs));

        $for_opening_balance = false;
        if ($request->get('for_opening_balance') == true) {
            $for_opening_balance = true;
        }
        //Distribute above payment among unpaid transactions
        $excess_amount = $this->payAtOnce($parent_payment, $due_payment_type, $for_opening_balance);
    ...
    public function updatePurchaseOrderStatus($purchase_order_ids = [])
    {
        ...
        foreach ($purchase_order_ids as $purchase_order_id) {
            ...
            $total_received = $purchase_order->purchase_lines->sum('qc_qty');
