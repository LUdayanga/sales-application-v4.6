    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('product.parameters')) {
                            $sub->url(
                                action('ProductParameterController@index'),
                                __('Product Parameters'),
                                ['icon' => 'fa fas fa-barcode', 'active' => request()->segment(1) == 'product-parameters']
                            );
                        }
                        if (auth()->user()->can('product.create')) {
                            $sub->url(
            ...
            if (auth()->user()->can('qc.view') || auth()->user()->can('qc.create')) {
                $menu->url(action('QualityController@index'), __('Quality Control'), ['icon' => 'fa fas fa-search-plus', 'active' => request()->segment(1) == 'backup'])->order(40);
            }
            
            //stores dropdown
            if (auth()->user()->can('stores.create')) {
                $menu->url(action('StoresController@index'), __('Stores for MRN'), ['icon' => 'fa fas fa-store', 'active' => request()->segment(1) == 'backup'])->order(45);
            }

            //machines dropdown
            if (auth()->user()->can('machines.view') || auth()->user()->can('machines.create')) {
                $menu->url(action('MachineController@index'), __('machines.machines'), ['icon' => 'fa fas fa-tools', 'active' => request()->segment(1) == 'backup'])->order(65);
            }

            //accounting module
            $menu->url(action('\Modules\Accounting\Http\Controllers\DashboardController@index'), '<span style="margin-left: 12px;">Accounting</span>', ['icon' => 'fas fa-wallet',  'active' => request()->segment(1) == 'backup'])->order(49);
            ...
            if (auth()->user()->can('account.access') && in_array('account', $enabled_modules)) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        $sub->url(
                            action('ChequesController@index'),
                            __('account.cheque_management'),
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'account' && request()->segment(2) == 'cheque']
                        );
                        $sub->url(
            ...
            if (auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view')
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        if (auth()->user()->can('product_requirement.view')) {
                            $sub->url(
                                action('ReportController@getQuickRequirement'),
                                __('Product Requirement'),
                                ['icon' => 'fa fas fa-percent', 'active' => request()->segment(2) == 'quick-requirement']
                            );
                        }
            ...
            if (auth()->user()->can('backup')) {
                $menu->url(action('BackUpController@index'), __('lang_v1.backup'), ['icon' => 'fa fas fa-hdd', 'active' => request()->segment(1) == 'backup'])->order(70);
            }
            ...
            if (in_array('booking', $enabled_modules) && (auth()->user()->can('crud_all_bookings') || auth()->user()->can('crud_own_bookings'))) {
                $menu->url(action('Restaurant\BookingController@index'), __('restaurant.bookings'), ['icon' => 'fas fa fa-calendar-check', 'active' => request()->segment(1) == 'bookings'])->order(75);
            }

            //Kitchen menu
            if (in_array('kitchen', $enabled_modules)) {
                $menu->url(action('Restaurant\KitchenController@index'), __('restaurant.kitchen'), ['icon' => 'fa fas fa-fire', 'active' => request()->segment(1) == 'modules' && request()->segment(2) == 'kitchen'])->order(80);
            }

            //Service Staff menu
            if (in_array('service_staff', $enabled_modules)) {
                $menu->url(action('Restaurant\OrderController@index'), __('restaurant.orders'), ['icon' => 'fa fas fa-list-alt', 'active' => request()->segment(1) == 'modules' && request()->segment(2) == 'orders'])->order(85);
            }

            //Notification template menu
            if (auth()->user()->can('send_notifications')) {
                $menu->url(action('NotificationTemplateController@index'), __('lang_v1.notification_templates'), ['icon' => 'fa fas fa-envelope', 'active' => request()->segment(1) == 'notification-templates'])->order(90);
            }
