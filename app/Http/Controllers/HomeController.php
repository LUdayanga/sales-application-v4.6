    ...
    public function getCheckAlert()
    {
        if (\request()->ajax()){
            ...
            $cheques = Cheque::join('accounts', 'cheques.account_id', '=', 'accounts.id')
                ->join('cheque_transactions', 'cheque_transactions.cheque_id', '=', 'cheques.id')
                ->where('cheques.cheque_status', 'due')
                ->whereRaw("DATEDIFF(DATE_ADD(cheques.cheque_date, INTERVAL 7 DAY), '$today') <= 7")
                ->select(
                    'cheques.id as id',
                    'accounts.name',
