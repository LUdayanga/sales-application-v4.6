...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getBusinessSettings()
    {
        ...
        $system_curenncies = SystemCurrencies::get();

        return view('business.settings', compact('business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'system_curenncies'));
    }
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            DB::table('system_currencies')->where('is_show_currency', 1)->delete();
            
            for ($i = 0; $i < count($request->currency_name); $i++) {
                if (!empty($request->currency_name[$i]) && !empty($request->currency_symbol[$i]) && !empty($request->currency_rate[$i])) {
                    if($request->is_show_currency[$i] != 0){
                        $SystemCurrencies = new SystemCurrencies;
                        $SystemCurrencies->currency_name = $request->currency_name[$i];
                        $SystemCurrencies->currency_symbol = $request->currency_symbol[$i];
                        $SystemCurrencies->currency_rate = $request->currency_rate[$i];
                        $SystemCurrencies->is_show_currency = 1;
                        $SystemCurrencies->save();
                    }
                }
            }
