...
class ReportController extends Controller
{
    ...
    public function getQuickRequirement(Request $request)
    {

        if (request()->ajax()) {

            $query = Product::join('units', 'products.unit_id', '=', 'units.id')
                ->leftJoin('transaction_sell_lines as tr_sell', 'products.id', '=', 'tr_sell.product_id')
                ->leftJoin('transactions as tr', 'tr.id', '=', 'tr_sell.transaction_id')
                ->where('tr.type', 'sales_order')
                ->where('tr.status', 'ordered');

            $products = $query->select(
                'products.name as product',
                'products.sku as sku',
                'units.short_name as unit',
                DB::raw("(SELECT SUM(qty_available) FROM variation_location_details where product_id = products.id) as current_stock"),
                DB::raw("(SELECT SUM(transaction_sell_lines.quantity) FROM transaction_sell_lines INNER JOIN transactions ON transactions.id = transaction_sell_lines.transaction_id where transaction_sell_lines.product_id = products.id AND transactions.type = 'sales_order' AND transactions.status = 'ordered') as order_quantity")
            )->groupBy('tr_sell.product_id')
            ->get();
                ...
                ->editColumn('current_stock', function ($row) {
                    if ($row->product != null) {
                        if ($row->current_stock == null) {
                            $current_stock = '0  ' .  $row->unit;
                            return $current_stock;
                        } else {
                            return round($row->current_stock, 2) . ' ' . $row->unit;
                        }
                    }
                })
                ->editColumn('order_quantity', function ($row) {
                    if ($row->product != null) {
                        return  round($row->order_quantity, 2) . ' ' . $row->unit;
                    }
                })
                ->editColumn('remaining', function ($row) {
                    if ($row->product != null) {
                        if (($row->current_stock != null || $row->order_quantity != null) && ($row->current_stock < $row->order_quantity)) {
                            return ($row->order_quantity - $row->current_stock) . ' ' . $row->unit;
                        } else {
                            $remaining = '0  ' . $row->unit;
                            return $remaining;
                        }
                    }
                })
        ...
        return view('report.quick_requirement');
    }

    public function getIngrediantRequirement(Request $request)
    {

        if (request()->ajax()) {

            $query = Product::join('units', 'products.unit_id', '=', 'units.id')
                ->join('mfg_recipe_ingredients as mfg', 'products.id', '=', 'mfg.variation_id');

            $products = $query->select(
                'products.name as product',
                'products.sku as sku',
                'units.short_name as unit',
            )->groupBy('mfg.variation_id')
            ->get();
            ...
            return Datatables::of($products)
                ...
                ->editColumn('product_current_stock', function ($row) {
                    ...
                    //     }
                    // }
                    $current_stock = '920 ' .  $row->unit;
                    return $current_stock;
                })
                ->editColumn('product_order_quantity', function ($row) {
                    // if ($row->product != null) {
                    //     return  round($row->order_quantity, 2) . ' ' . $row->unit;
                    // }
                    $current_stock = '920 ' .  $row->unit;
                ...
                ->editColumn('product_remaining', function ($row) {
                    // if ($row->product != null) {
                    //     if (($row->current_stock != null || $row->order_quantity != null) && ($row->current_stock < $row->order_quantity)) {
                    //         return ($row->order_quantity - $row->current_stock) . ' ' . $row->unit;
                    //     } else {
            ...
            $datatable =  Datatables::of($products)
                ->editColumn('stock', function ($row) {
                    ...
                    if ($row->enable_stock) {
                        ...
                        $html .= '<span data-is_quantity="true" class="current_stock display_currency" data-orig-value="' . (float)$stock . '" data-unit="' . $row->unit . '" data-currency_symbol=false > ' . (float)$stock . '</span>' . ' ' . $row->unit;
                    } else {
                        return '--';
                    }
                    // $html .='<button type="button" class="btn btn-danger btn-xs btn-modal no-print" data-container=".view_modal" data-href="' . action('ProductController@viewLotGroup', [$row->product_id]) .'">' . __('View lots group') . '</button>';
        ...
        if ($request->ajax()) {
            ...
            $query = PurchaseLine::join(
                ...
                ->select(
                    ...
                    'purchase_lines.qc_qty as purchase_qc_qty',
                    'purchase_lines.lot_number as purchase_lot_number',
                    ...
                    DB::raw('((purchase_lines.qc_qty - purchase_lines.quantity_returned - purchase_lines.quantity_adjusted) * purchase_lines.purchase_price_inc_tax) as qctotal')
                )
            ...
            return Datatables::of($query)
                ...
                ->editColumn('purchase_qc_qty', function ($row) {
                    return '<span data-is_quantity="true" class="display_currency purchase_qc_qty" data-currency_symbol=false data-orig-value="' . (float)$row->purchase_qc_qty . '" data-unit="' . $row->unit . '" >' . (float) $row->purchase_qc_qty . '</span> ' . $row->unit;
                })
                ...
                ->editColumn('qctotal', function ($row) {
                    return '<span class="display_currency row_qctotal" data-currency_symbol=true data-orig-value="' . $row->qctotal . '">' . $row->qctotal . '</span>';
                })
                ...
                ->rawColumns(['ref_no', 'unit_purchase_price', 'subtotal', 'qctotal', 'purchase_qty', 'purchase_lot_number', 'purchase_qc_qty', 'quantity_adjusted', 'supplier'])
                ->make(true);
        }
